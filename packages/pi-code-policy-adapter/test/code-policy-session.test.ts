...
test("code-policy facade rejects any activated tool inventory drift", () => {
  assert.doesNotThrow(() => assertCodePolicyToolInventory(["python_exec"]));
  assert.throws(() => assertCodePolicyToolInventory([]), /exactly python_exec/);
  assert.throws(
    () => assertCodePolicyToolInventory(["python_exec", "read"]),
    /exactly python_exec/,
  );
});
