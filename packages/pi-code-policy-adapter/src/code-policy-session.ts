...
export function assertCodePolicyToolInventory(names: readonly string[]): void {
  if (names.length !== 1 || names[0] !== CODE_POLICY_TOOL_NAME) {
...
export async function createFreshCodePolicySession(
  ...
  try {
    assertCodePolicyToolInventory(result.activeToolNames);
  } catch (error) {
