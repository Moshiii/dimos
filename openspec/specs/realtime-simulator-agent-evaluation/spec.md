# realtime-simulator-agent-evaluation Specification

## Purpose

Define canonical case-driven real-time simulator evaluation through `dimos eval
run`, including evaluator-owned lifecycle, private goal control, evidence, and
...
The real-time evaluator SHALL honor the resolved DimOS viewer and Rerun Web presentation settings while preserving all case-bound source, interaction, and validator semantics. Visualization settings MUST NOT participate in case identity or private evaluation truth, and `--viewer none` SHALL remain available for unattended execution.
