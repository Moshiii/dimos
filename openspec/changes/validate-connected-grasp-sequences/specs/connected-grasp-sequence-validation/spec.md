...
### Requirement: Offline full-pipeline demo
The repository SHALL provide a no-hardware CLI demo that runs a recorded target-only cloud through GraspGenX proposal generation and connected RoboPlan validation. The demo SHALL stop before execution and SHALL save candidate outcomes, the selected candidate, and all connected joint paths as inspectable artifacts.

#### Scenario: Contributor runs the pipeline demo
- **WHEN** the recorded fixture, GraspGenX runtime, model, CUDA, and xArm planning data are available
