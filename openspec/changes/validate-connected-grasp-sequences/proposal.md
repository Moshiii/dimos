...
- Add deterministic unit and backend-contract coverage plus a compact GraspGenX-to-planner integration test strategy.
- Add a no-hardware CLI demo that runs the complete proposal-and-planning pipeline and saves inspectable candidate and path artifacts.
...
- Adds no new runtime dependency; GPU-backed GraspGenX validation remains guarded by the existing optional dependency and test environment.
