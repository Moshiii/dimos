...
def make_module(
    ...
    def _make(
        ...
        *,
        ...
        mocker.patch(
            "dimos.experimental.robot_policy.lerobot._load_policy_backend",
            side_effect=_load,
        )
        module = module_type(
            policies=policies,
...
def test_policy_observation_and_action_use_canonical_order(
    ...
    assert backend.robot_type == "test_arm"
...
def test_invalid_policy_action_stops_without_publishing(
    ...
    assert f"expected ({len(JOINTS)},)" in status["last_error"]
...
def test_policy_refuses_stale_observations_without_loading(
    make_module: ModuleFactory,
) -> None:
    backend = FakeBackend(np.zeros(len(JOINTS), dtype=np.float32))
    module, output, _finished = make_module({"default": backend})
...
def test_policy_refuses_incomplete_joint_state_without_loading(
    make_module: ModuleFactory,
) -> None:
    backend = FakeBackend(np.zeros(len(JOINTS), dtype=np.float32))
    module, output, _finished = make_module({"default": backend})
...
def test_policy_stops_when_action_contains_non_finite_value(
    make_module: ModuleFactory,
) -> None:
...
def test_concurrent_policy_start_is_rejected(
    make_module: ModuleFactory,
) -> None:
    cup = FakeBackend(np.zeros(len(JOINTS), dtype=np.float32))
    plate = FakeBackend(np.zeros(len(JOINTS), dtype=np.float32))
    ...
    assert plate.reset_count == 0
    assert module.stop_learned_policy() == "Learned policy stopped."


def test_policy_reports_duration_completion(
    make_module: ModuleFactory,
) -> None:
    backend = FakeBackend(np.zeros(len(JOINTS), dtype=np.float32))
    module, output, finished = make_module({"default": backend})
    _provide_observation(module)
...
def test_backend_reports_optional_dependency_install_command(
    mocker: pytest_mock.MockerFixture,
) -> None:
    ...
    policy = config.policies["default"]
    mocker.patch(
        "dimos.experimental.robot_policy.lerobot.import_module",
        side_effect=ImportError("missing optional dependency"),
    )

    with pytest.raises(ImportError, match=r"uv sync --extra lerobot"):
        _LeRobotBackend(config, policy)
