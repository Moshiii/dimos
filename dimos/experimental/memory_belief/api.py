...
def execute(store: Any, query: dict[str, Any]) -> dict[str, Any]:
    ...
    if not rows:
        return {
            ...
            "quality": {"rows": 0},
            "time_base": {"as_of": as_of, "unit": "seconds"},
            "diagnostic": _diagnose(store, query),
            "query": query,
        }
...
def _diagnose(store: Any, query: dict[str, Any]) -> dict[str, Any]:
    ...
    stream = _open(store, query["select"])
    remaining = len(stream.to_list())
    steps = []
    for clause in where:
        try:
            stream = _apply(stream, [clause])
            after = len(stream.to_list())
