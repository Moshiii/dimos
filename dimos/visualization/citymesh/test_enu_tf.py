...
OFFSET = (10.0, -5.0, 2.0)  # where odometry says the robot is, relative to ENU
...
def _odom_for(fix: NavSatFix, frame: EnuFrame) -> Odometry:
    e, n, u = frame.geodetic_to_enu(fix.latitude, fix.longitude, fix.altitude, datum="msl")[0]
    return Odometry(pose=Pose(float(e) + OFFSET[0], float(n) + OFFSET[1], float(u) + OFFSET[2]))


def test_no_fix_or_no_odom_yields_nothing():
    snap = EnuSnap()
    assert snap.add_fix(NavSatFix(status=NavSatFix.STATUS_NO_FIX)) is None
    assert snap.add_fix(_fix()) is None, "without odometry there is no offset to measure"
...
def test_transform_is_the_offset_with_identity_yaw():
    ...
    for i in range(3):
        ...
        snap.add_odom(_odom_for(fix, frame))
        transform = snap.add_fix(fix)
...
def test_needs_min_samples_before_speaking():
    ...
    snap.add_odom(_odom_for(fix, frame))
    for _ in range(4):
        assert snap.add_fix(fix) is None
    assert snap.add_fix(fix) is not None
...
def test_median_shrugs_off_an_outlier_fix():
    ...
    frame = _frame()
    fix = _fix()
    odom = _odom_for(fix, frame)
    snap.add_odom(odom)
    for _ in range(5):
        snap.add_fix(fix)
    # One wild fix, same odometry: a mean would drag the origin metres away.
    transform = snap.add_fix(_fix(lat=ATHENS[0] + 3e-4))
    assert transform is not None
...
def test_frame_anchors_at_the_snapped_origin():
    snap = EnuSnap()
    fix = _fix()
    snap.add_odom(Odometry(pose=Pose(0.0, 0.0, 0.0)))
    snap.add_fix(fix)
...
def test_nan_altitude_measures_z_against_sea_level():
    ...
    fix = _fix(alt=float("nan"))
    snap.add_odom(Odometry(pose=Pose(0.0, 0.0, 1.5)))
    transform = snap.add_fix(fix)
    assert transform is not None
