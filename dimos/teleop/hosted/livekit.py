...
class LiveKitTeleopModule(Module):
    ...
    def _run(self) -> None:
        ...
        with self._lock:
            self._loop = loop
        try:
            loop.run_until_complete(self._run_room())
        finally:
    ...
    async def _connect_room(self, session: LiveKitSession) -> None:
        ...
        def _data_received(packet: Any) -> None:
            self._on_data(getattr(packet, "topic", "") or "", bytes(packet.data))
    ...
    def _is_operator(self, participant: Any) -> bool:
        identity = getattr(participant, "identity", None)
        return self.config.operator_identity is None or identity == self.config.operator_identity
    ...
    def _on_data(self, topic: str, data: bytes) -> None:
