...
class QuestTeleopConfig(ModuleConfig):
    ...
    # When set, every websocket frame the headset sends (pose, joy) is
    # appended verbatim to this JSONL file with receive timestamps, so a
    # session can be replayed bit-exactly against a sim blueprint.
    # strftime tokens expand when the file opens. Empty disables.
    record_path: str = ""
...
class QuestTeleopModule(Module):
    ...
    def _record_frame(self, data: bytes) -> None:
        ...
        with self._record_lock:
            ...
            # Survive a crash with at most about a second of frames lost.
