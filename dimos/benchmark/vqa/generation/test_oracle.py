...
def test_private_oracle_rejects_unsupported_measurement_claim() -> None:
    class _NumericModel(_BoundModel):
        def invoke(self, messages: Any) -> AIMessage:
            self._calls += 1
            if self._calls == 1:
                return AIMessage(
            ...
            return AIMessage(content='{"answer":1.2,"evidence_ids":["grounding:v1:chair-1"]}')

    proposal = QuestionProposal("q", "How tall is the chair?", NumericAnswerContract("m", 0.1))
    registry = LocalOracleToolRegistry(cast("Any", object()), cast("Any", _Grounding()))
    ...
    result = PrivateToolCallingOracle(
        cast("Any", _NumericModel()), semantic_validator=validator
    ).answer(proposal, registry)
