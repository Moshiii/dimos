        ...
        response = self._model.invoke(
            [
                SystemMessage(
                    ...
                    "height is not supported by range or side alone. For numeric answers, require "
                    "a cited numeric measurement with matching unit and an answer within its stated "
                    "tolerance. Return strict JSON only: "
...
def validate_oracle_answer(
    ...
    if isinstance(contract, NumericAnswerContract):
        if not isinstance(answer, (int, float)) or isinstance(answer, bool):
            raise ValueError("numeric answer must be a number")
...
def _contract_prompt(contract: AnswerContract) -> str:
    ...
    return f"numeric: unit={contract.unit}, tolerance={contract.tolerance}"
