...
def _proposal_from_json(item: Any, index: int) -> QuestionProposal:
    ...
    if kind == "boolean":
        ...
        unit, tolerance = contract.get("unit"), contract.get("tolerance")
        if (
            ...
            raise ValueError("numeric contract requires unit and non-negative tolerance")
        answer_contract = NumericAnswerContract(unit, float(tolerance))
    else:
