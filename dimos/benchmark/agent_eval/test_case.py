...
def test_native_validator_score_does_not_require_a_text_prediction() -> None:
    score = PrivateScore(
        case_id="case",
        attempt_id="attempt_" + "a" * 32,
        validator_revision="native-v1",
        passed=True,
        prediction_status="native",
    )

    assert score.prediction_status == "native"


def test_hongkong_public_northstar_and_agent_condition_are_strict() -> None:
    root = Path(__file__).parents[1] / "short_horizon_qa" / "cases"
    public = PublicEvalCase.model_validate_json(
        (root / "go2_hongkong_office-room-count.public.json").read_bytes()
    )
    agent = AgentCondition.model_validate_json(
        (root / "go2_hongkong_office-room-count.agent.json").read_bytes()
    )

    assert public.source.kind == "frozen_memory"
    assert public.source.progress == 1.0
    assert public.task.prompt == "How many rooms in total?"
