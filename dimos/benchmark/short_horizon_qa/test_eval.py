...
class ScriptedPiFactory:
    ...
    def create(
        ...
        *,
        ...
        self.public_prompts.append(public_prompt)
...
def test_real_standalone_frozen_attempt_scores_scripted_pi(
    ...
    oracle_sentinel = "ORACLE_PRIVATE_SENTINEL_7f9d"
    credential_sentinel = "CREDENTIAL_PRIVATE_SENTINEL_a13c"
    oracle = {
        "schema_version": "1.0",
        "expected_count": 2,
        "counting_policy": oracle_sentinel,
        "rooms": [
    ...
    factory = ScriptedPiFactory(final_text)
        ...
        runtime_binding=RuntimeBinding(
            ...
            parameters={
                "credential_binding_sha256": credential_binding_sha256(
                    "environment", "TEST_API_KEY", credential_sentinel
                )
            },
        ),
    )
    ...
    # The private oracle is intentionally retained in its private artifact. Every
    # model-facing or public/runtime surface must remain sentinel-free.
    public_surfaces = [
        case.public_projection().model_dump_json(),
        *factory.public_prompts,
        result.outcome.model_dump_json(),
    ]
    for path in result.attempt_path.rglob("*"):
        if path.is_file() and path.name not in {
            "oracle.private.v1.json",
            "score.private.v1.json",
        }:
            public_surfaces.append(path.read_text(errors="replace"))
    serialized = "\n".join(public_surfaces)
    assert oracle_sentinel not in serialized
