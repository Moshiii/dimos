...
def validate_native_result(
    ...
    *,
    ...
    if not isinstance(source, ExternalBenchmarkEpisodeSource) or not isinstance(
        ...
        raise TypeError("native VLN-CE validation requires an external benchmark case")
    schema_digest = hashlib.sha256(schema_path.read_bytes()).hexdigest()
    if schema_digest != validator.result_schema_sha256:
        raise ValueError("native result schema does not match the case-bound digest")
    try:
