...
class VlnceExternalRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        source = case.source
        task = case.task
        if not isinstance(source, ExternalBenchmarkEpisodeSource) or not isinstance(
            task, BenchmarkInstructionTask
        ):
            raise TypeError("VLN-CE runtime requires an external benchmark case")
        if not isinstance(case.interaction, LiveCodePolicyInteraction):
            raise TypeError("VLN-CE runtime requires a live CodePolicy interaction")
        self.case = case
        self.source = source
        self.task = task
    ...
    def start(self, readiness_timeout_seconds: float = 180.0) -> dict[str, JsonValue]:
        ...
        while not self.memory_path.exists():
            self._raise_if_container_exited("while waiting for live memory")
            if time.monotonic() >= deadline:
                raise TimeoutError("VLN-CE live memory did not become ready")
            time.sleep(0.05)
        ...
        return cast(
            "dict[str, JsonValue]",
            {
                ...
                "initial_observation_ready": True,
                "memory_ready": True,
    ...
    def _prepare_cdi(self) -> tuple[list[str], list[str]]:
        ...
        specification = self.cdi_dir / "nvidia.yaml"
        result = subprocess.run(
            [nvidia_ctk, "cdi", "generate", f"--output={specification}"],
            capture_output=True,
            check=False,
        )
