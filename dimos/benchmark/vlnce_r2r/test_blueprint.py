...
def test_blueprint_wires_static_map_motion_and_public_recording(tmp_path) -> None:
    ...
    assert (
        blueprint.remapping_map[(AgentEvalObservationRecorder.name, "global_map")] == "pointcloud"
    )
...
def test_viewer_choice_changes_only_public_presentation_modules(
    ...
    fingerprint_before = EvalCase.model_validate_json(case_path.read_bytes()).fingerprint
    ...
    modules = {atom.module for atom in blueprint.blueprints}
    fingerprint_after = EvalCase.model_validate_json(case_path.read_bytes()).fingerprint
