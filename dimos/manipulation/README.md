...
All three use the wrist-mounted RealSense and object-scene registration in
`link_base`. The GraspGenX variants automatically choose the first score-ordered
candidate that has a connected, collision-aware pre-grasp, grasp, and retreat
motion sequence.
...
3. Select `3` and choose an object. The GraspGenX blueprint evaluates proposals
   in model-score order and automatically selects the first fully feasible one.
   The console prints the proposals and marks the selected rank. Viser shows the
   selected object cloud in amber, the grasp TCP axes in red, and the pre-grasp
   TCP axes in green.
...
`PickNPlaceModule.get_goal_pose()` asks `ManipulationModule` to evaluate
GraspGenX candidates in score order. It stores the first candidate for which
collision-aware IK and planning succeed for the connected pre-grasp, grasp, and
retreat sequence. The selected TCP goal remains in the candidate point cloud's
frame. Its pre-grasp is computed as:
...
length. Candidate score order comes from GraspGenX; the operator pipeline adds
motion-feasibility filtering but does not rerank the proposals.

## Implementation Guide

- `blueprints.py`: robot, camera, OBB, and GraspGenX blueprint composition.
- `picknplace.py`: scan request, target selection, OBB fallback, and delegation
  to the shared motion-feasibility selector.
...
and fused object clouds are planned follow-up work. Until then, select targets
with a complete enough visible point cloud for grasping.
