...
def _print_grasp_candidates(candidates: Any) -> None:
    ...
    for rank, candidate in enumerate(candidates.candidates[:10]):
        pose = candidate.pose
        selected = " SELECTED" if rank == candidates.selected_index else ""
        print(
            f"{rank}: score={candidate.score:.3f} position={pose.position.as_tuple} "
            f"orientation={pose.orientation.to_tuple()}{selected}"
        )
...
def main() -> None:
    ...
    while True:
        ...
        print(
            "9) Plan/preview ascent  10) Execute ascent  11) Open  12) Current EE  13) Go home  q) Quit"
        )
