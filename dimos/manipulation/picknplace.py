...
from dimos.manipulation.grasping.grasp_selection import GraspSelectionSpec
...
class PickNPlaceModule(Module):
    ...
    _manipulation: GraspSelectionSpec
    ...
    def get_goal_pose(self, number: int) -> PoseStamped | None:
        ...
        if self.config.grasp_strategy == "graspgenx":
            ...
            proposal_input = GraspProposalInput.from_pointcloud(obj.pointcloud)
            candidates = self._grasp_generator.propose_grasps(proposal_input)
            self._selected_object = obj
            self._grasp_candidates = candidates
            if not candidates.candidates:
                self.graspgenx_candidates.publish(candidates)
                return None
            selection = self._manipulation.select_first_feasible_grasp(
                candidates,
                "arm",
                self.config.graspgenx_pregrasp_offset,
                self.config.graspgenx_pregrasp_offset,
            )
            ...
            candidates.selected_index = selected.rank - 1
            self.graspgenx_candidates.publish(candidates)
            self._goal_pose = PoseStamped(
                ts=candidates.header.timestamp,
                frame_id=candidates.header.frame_id,
                position=selected.candidate.pose.position,
                orientation=selected.candidate.pose.orientation,
            )
            self._pre_grasp_pose = PoseStamped(
                ts=candidates.header.timestamp,
                frame_id=candidates.header.frame_id,
                position=selected.pre_grasp_pose.position,
                orientation=selected.pre_grasp_pose.orientation,
            )
    ...
    def get_pre_grasp_pose(self) -> PoseStamped | None:
        ...
        if self._goal_pose is None:
            return None
        if self._pre_grasp_pose is not None:
