.. _doc-usage-modules--dimos-modules:
...
.. _doc-usage-modules--camera-module:
...
- ``color_image`` with `sensor_msgs.Image <https://docs.ros.org/en/melodic/api/sensor_msgs/html/msg/Image.html>`__ type
- ``camera_info`` with `sensor_msgs.CameraInfo <https://docs.ros.org/en/melodic/api/sensor_msgs/html/msg/CameraInfo.html>`__ type

It offers two RPC calls: ``start()`` and ``stop()`` (lifecycle methods).

It also exposes an agentic :ref:`skill <doc-usage-blueprints--defining-skills>` called ``take_a_picture`` (more on skills in the Blueprints guide).
...
.. _doc-usage-modules--connecting-modules:
...
.. _doc-usage-modules--distributed-execution:
...
.. _doc-usage-modules--dedicated-workers:
...
.. _doc-usage-modules--sync-input-handlers:
...
.. _doc-usage-modules--triggering-side-effects-via-specs:
...
.. _doc-usage-modules--testing-modules:
...
.. _doc-usage-modules--restarting-a-module:
...
.. _doc-usage-modules--async-modules-lock-free-state:
...
.. _doc-usage-modules--main-combined-setupteardown:
...
To see more information on how to use Blueprints, see :doc:`Blueprints </usage/blueprints>`.

.. |output| image:: assets/go2_nav.svg
.. |image1| image:: assets/go2_agentic.svg
