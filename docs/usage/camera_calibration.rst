.. _doc-usage-camera-calibration--camera-calibration:

==================
Camera calibration
==================

.. _doc-usage-camera_calibration--camera-calibration:
...
.. _doc-usage-camera_calibration--print-and-measure-the-chessboard:
...
1. **Generate a checkerboard.** OpenCV documents pattern creation in `Create calibration pattern <https://docs.opencv.org/4.12.0/da/d0d/tutorial_camera_calibration_pattern.html>`__. To draw your own board, download and run `OpenCV's gen_pattern.py <https://github.com/opencv/opencv/blob/4.12.0/doc/pattern_tools/gen_pattern.py>`__ from that OpenCV version (see the tutorial for dependencies). In that script, ``--columns`` and ``--rows`` count **checker squares** along each axis. For **8 by 6 inner corners**, use **9 columns and 7 rows** of squares (inner corners are one less than square count in each direction):
...
.. _doc-usage-camera_calibration--capture-practice:
...
.. _doc-usage-camera_calibration--run-dimos-cameracalibrate:
...
.. _doc-usage-camera_calibration--distortion-model-pinhole-vs-fisheye:
...
.. _doc-usage-camera_calibration--verify-the-yaml:
...
1. Open the file and confirm ``image_width`` and ``image_height`` match the resolution your camera actually delivers in the stack (same width and height as your calibration images or as your :class:`Webcam <dimos.hardware.sensors.camera.webcam.Webcam>` width and height settings). Wrong dimensions mean intrinsics are being applied to the wrong raster shape.

2. Confirm ``distortion_model`` is ``plumb_bob``. dimos :func:`load_camera_info <dimos.perception.common.utils.load_camera_info>` reads this key (not ``camera_model``); other values are only valid if every consumer in your pipeline agrees on the same model.

3. ``frame_id``: the YAML written by ``dimos cameracalibrate`` does not embed ``frame_id``. When you build a :class:`CameraInfo <dimos.msgs.sensor_msgs.CameraInfo.CameraInfo>` from the file (for example ``load_camera_info(path, frame_id=...)``), the ``frame_id`` you pass must match the ``Image.header.frame_id`` used for that camera in your graph. The stock :class:`Webcam <dimos.hardware.sensors.camera.webcam.Webcam>` publishes color images with frame id ``camera_optical`` (or ``{frame_id_prefix}/camera_optical`` if :attr:`frame_id_prefix <dimos.core.module.ModuleConfig.frame_id_prefix>` is set). The no-robot desk blueprint (ticket T5 in the feature backlog) publishes TF so that optical frame is consistent with that naming; keep your loaded :class:`CameraInfo <dimos.msgs.sensor_msgs.CameraInfo.CameraInfo>` header aligned with whatever your desk stack actually publishes for the optical camera.

For the shape of the ROS-style fields only (matrix blocks, row and column counts, key names), see the checked-in example `dimos/hardware/sensors/camera/zed/single_webcam.yaml <https://github.com/dimensionalOS/dimos/blob/main/dimos/hardware/sensors/camera/zed/single_webcam.yaml>`__. Treat it as a schema reference, not as calibration numbers for your camera.
