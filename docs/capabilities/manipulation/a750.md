...
The runtime model and `a750_description` package root are resolved through a Git-backed robot description source handle. The no-gripper model remains LFS-backed for Pinocchio FK/IK inside the coordinator's EEF twist task because upstream publishes the full gripper URDF but not the generated no-gripper variant. Keyboard teleop does not load the model directly.
