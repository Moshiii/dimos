.. _doc-development-conventions--conventions:
...
- Instead of using ``RerunBridge`` in blueprints we always use :func:`vis_module <dimos.visualization.vis_module.vis_module>` which allows the CLI to control if its rerun or no-vis at all
...
- When adding visualization tools to a blueprint/autoconnect, instead of using RerunBridge or WebsocketVisModule directly we should always use :func:`vis_module <dimos.visualization.vis_module.vis_module>`, which right now should look something like ``vis_module(viewer_backend=global_config.viewer, rerun_config={}),``
